...
if ($funcionarioId === 0) {
    redirect('funcionarios-clientes.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $db->prepare("UPDATE funcionarios_clientes SET
            ...
            numero_dependentes = ?, status = ?, observacoes = ?
            WHERE id = ?
        ");
        ...
        $stmt->execute([
            ...
            $numeroDependentes, $status, $observacoes,
            $funcionarioId
        ]);
...
$breadcrumb = '<a href="dashboard.php">Dashboard</a> / <a href="funcionarios-clientes.php">Funcionários</a> / Editar';
...
<div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 mt-1">Atualizar dados do funcionário <?php echo htmlspecialchars($funcionario['nome_completo']); ?></p>
        </div>
        <div class="flex gap-2">
            <a href="funcionario-cliente-detalhes.php?id=<?php echo $funcionarioId; ?>" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors flex items-center gap-2">
                ...
                <div>
                    ...
                    <select name="cliente_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        ...
                        <?php foreach ($clientes as $c): ?>
                            <option value="<?php echo $c['id']; ?>" <?php echo $funcionario['cliente_id'] == $c['id'] ? 'selected' : ''; ?>>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="nome_completo" value="<?php echo htmlspecialchars($funcionario['nome_completo']); ?>" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">CPF *</label>
                    <input type="text" name="cpf" value="<?php echo htmlspecialchars($funcionario['cpf']); ?>" required maxlength="14" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="000.000.000-00">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">RG</label>
                    <input type="text" name="rg" value="<?php echo htmlspecialchars($funcionario['rg'] ?? ''); ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Data de Nascimento</label>
                    <input type="date" name="data_nascimento" value="<?php echo $funcionario['data_nascimento']; ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                </div>
                <div>
                ...
                <div class="md:col-span-3">
                    ...
                    <select name="estado_civil" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        <option value="">Selecione</option>
                        <option value="Solteiro(a)" <?php echo $funcionario['estado_civil'] === 'Solteiro(a)' ? 'selected' : ''; ?>>Solteiro(a)</option>
                        <option value="Casado(a)" <?php echo $funcionario['estado_civil'] === 'Casado(a)' ? 'selected' : ''; ?>>Casado(a)</option>
                        <option value="Divorciado(a)" <?php echo $funcionario['estado_civil'] === 'Divorciado(a)' ? 'selected' : ''; ?>>Divorciado(a)</option>
                        <option value="Viúvo(a)" <?php echo $funcionario['estado_civil'] === 'Viúvo(a)' ? 'selected' : ''; ?>>Viúvo(a)</option>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                <div>
                    ...
                    <select name="status" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                        ...
                        <option value="Férias" <?php echo $funcionario['status'] === 'Férias' ? 'selected' : ''; ?>>Férias</option>
                        <option value="Afastado" <?php echo $funcionario['status'] === 'Afastado' ? 'selected' : ''; ?>>Afastado</option>
                        <option value="Demitido" <?php echo $funcionario['status'] === 'Demitido' ? 'selected' : ''; ?>>Demitido</option>
                    </select>
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Observações</label>
                <textarea name="observacoes" rows="4" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent" placeholder="Informações adicionais sobre o funcionário..."><?php echo htmlspecialchars($funcionario['observacoes'] ?? ''); ?></textarea>
            </div>
        </div>

        <!-- Botões -->
        <div class="flex items-center justify-end gap-4">
            <a href="funcionario-cliente-detalhes.php?id=<?php echo $funcionarioId; ?>" class="px-6 py-3 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
                Cancelar
            </a>
