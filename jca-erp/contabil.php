...
$whereUser = !isAdmin() ? " AND responsavel_id = " . (int)$usuario['id'] : "";
$pend = (int)$db->query("SELECT COUNT(*) FROM tarefas WHERE tipo='Contábil' AND status IN ('Pendente','Em Andamento','Aguardando'){$whereUser}")->fetchColumn();
$concl = (int)$db->query("SELECT COUNT(*) FROM tarefas WHERE tipo='Contábil' AND status='Concluída'{$whereUser}")->fetchColumn();

$stmt = $db->prepare("SELECT t.*, c.razao_social as cliente_nome FROM tarefas t LEFT JOIN clientes c ON t.cliente_id=c.id
                      WHERE t.tipo='Contábil'" . (!isAdmin() ? " AND t.responsavel_id=?" : "") . " ORDER BY t.data_vencimento ASC, t.data_criacao DESC LIMIT 10");
$stmt->execute(!isAdmin() ? [$usuario['id']] : []);
$ult = $stmt->fetchAll();

$pageTitle='Módulo Contábil';
$breadcrumb='<a href="dashboard.php">Dashboard</a> / Contábil';
...
<div class="p-6 space-y-6">
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div class="bg-white rounded-lg border border-gray-200 p-6">
      <p class="text-sm text-gray-600 mb-2">Tarefas em aberto (Contábil)</p>
      <h3 class="text-3xl font-bold text-gray-900"><?php echo $pend; ?></h3>
    </div>
    <div class="bg-white rounded-lg border border-gray-200 p-6">
      <p class="text-sm text-gray-600 mb-2">Tarefas concluídas (Contábil)</p>
      <h3 class="text-3xl font-bold text-gray-900"><?php echo $concl; ?></h3>
    </div>
  </div>

  <div class="bg-white rounded-lg border border-gray-200 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
      <h5 class="text-lg font-semibold text-gray-900">Próximas tarefas contábeis</h5>
